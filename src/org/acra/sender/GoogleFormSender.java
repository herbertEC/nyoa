...
public class GoogleFormSender implements ReportSender {
	...
	public GoogleFormSender(String formKey) {
		mFormUri = Uri.parse(String.format(ACRA.getConfig()
				.googleFormUrlFormat(), formKey));
	}

	@Override
	public void send(CrashReportData report) throws ReportSenderException {
		Uri formUri = mFormUri == null ? Uri
				.parse(String.format(ACRA.getConfig().googleFormUrlFormat(),
						ACRA.getConfig().formKey())) : mFormUri;
		...
		try {
			...
			Log.d(LOG_TAG,
					"Sending report " + report.get(ReportField.REPORT_ID));
			...
			throw new ReportSenderException(
					"Error while sending report to Google Form.", e);
		}
	}
	...
	private Map<String, String> remap(Map<ReportField, String> report) {
		...
		for (ReportField originalKey : fields) {
			...
			case APP_VERSION_NAME:
				result.put("entry." + inputId + ".single",
						"'" + report.get(originalKey));
				break;
